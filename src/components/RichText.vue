...
<script>
// 引入serve文件
import serve from '@/api/file.api'
// 引入tinymce编辑器
...
export default {
  ...
  props: {
    ...
    plugins: {
      type: [String, Array],
      default:
        'preview searchreplace autolink directionality visualblocks visualchars fullscreen image link template codesample table charmap hr pagebreak nonbreaking insertdatetime advlist lists wordcount textpattern autosave '
    ...
    toolbar: {
      ...
      default:
        ...
        'fullscreen undo redo restoredraft | forecolor backcolor bold italic underline strikethrough link | alignleft aligncenter alignright alignjustify outdent indent | \
        styleselect formatselect fontselect fontsizeselect | bullist numlist | blockquote subscript superscript removeformat | \
        table image charmap hr pagebreak insertdatetime preview | searchreplace visualblocks | indent2em lineheight formatpainter axupimgs'
  ...
  data () {
    return {
      init: {
        ...
        // images_upload_base_path: this.IMAGEURL,  //相对基本路径--关于图片上传建议查看--http://tinymce.ax-z.cn/general/upload-images.php

        relative_urls: false, // 富文本中的图片是绝对路径，而不是相对路径。
        remove_script_host: false, // 富文本中的图片是绝对路径，而不是相对路径。
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          if (blobInfo.blob().size / 1024 / 1024 > 2) {
            ...
            serve.uploadSingleFile(params).then(res => {
              console.log(res, 'rich')
              if (res.data.code === 200) {
                const url = `${this.IMAGEURL}${res.data.data.filePath}`
